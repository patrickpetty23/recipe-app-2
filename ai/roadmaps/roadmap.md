...
- [x] Create `.gitignore` (covers secrets and library folders; `ai/` is tracked)
...
- [x] Fix Shopping List UI layout — flexbox refactor, remove duplicate header
- [x] Fix Walmart cart URL — use `affil.walmart.com/cart/addToCart` format
- [x] Fix `crypto` module error — use `node-forge` exclusively in React Native
- [x] Graceful Walmart API key handling — popup alert instead of crash
